...
//!
//! - 宿主以 `args = { messages, input, note_id, selection? }` 调 `chat` 命令，返回 `{ reply }`；
//!   消息历史 + 多会话由前端持有/持久化（localStorage），插件无状态（每次调用新实例）。
//! - `selection`（宿主 §9.2）= 用户当前在笔记内容区选中的文字：有则注入系统提示词，
//!   让 AI 优先处理这段（改写/翻译/精简走 `replace_in_note` 精确替换）。
//! - AI 要读写笔记库时按本插件的工具协议回复（整条回复只含一个 json 代码块），
//!   插件执行工具后把结果以 user 消息回喂、再调 `ai.complete`，最多 [`MAX_TOOL_ROUNDS`] 轮。
//!   工具：search_notes / read_note / list_folders / replace_in_note（精确替换）/ update_note / create_note。
...
/// 注入提示词的「当前选中文字」截断阈值（字符）：选区一般不长，但兜底防超长粘贴。
const SELECTION_MAX_CHARS: usize = 4_000;
...
- replace_in_note {"id"?:string,"old":string,"new":string}：把笔记正文里的一段文字 old **精确替换**为 new（只替第一处匹配）。改写选中段落/某句话时**优先用它**——比 update_note 整体替换更安全、不会动到其它内容；省略 id 改当前笔记
- update_note {"id"?:string,"title"?:string,"body"?:string}：修改笔记；省略 id 时改当前打开的笔记；只有给出的字段会被修改，body 是**整体替换**（大改动/换标题时用；小修用 replace_in_note）
- create_note {"parent_id":string,"title"?:string,"body"?:string}：在指定笔记本里新建笔记
工具结果会以一条用户消息回给你；之后你可以继续调工具，或给出最终回复。
如果下方提供了「当前选中的文字」，用户多半是想让你处理这段——改写/翻译/精简它就用 replace_in_note，old 填这段原文、new 填改写后的版本。
...
/// 组系统提示词：协议 + 当前笔记上下文 + 当前选中的文字。读当前笔记失败只降级说明，不让整次对话失败。
fn system_prompt(note_id: Option<&str>, selection: Option<&str>) -> String {
    ...
    if let Some(sel) = selection {
        p.push_str(&format!(
            "\n\n# 当前选中的文字\n{}\n（用户多半想让你处理这段——改写它就用 replace_in_note，old 填这段原文）",
            clip_to(sel, SELECTION_MAX_CHARS)
        ));
    }
    p
}
...
fn run_tool(name: &str, args: &Value, note_id: Option<&str>) -> Result<Value, PluginError> {
    match name {
        ...
        "replace_in_note" => {
            let id = arg_str(args, "id")
                .or(note_id)
                .ok_or_else(|| PluginError::invalid("replace_in_note 缺 id，且当前没有打开的笔记"))?;
            let old = arg_str(args, "old")
                .ok_or_else(|| PluginError::invalid("replace_in_note 需要非空 old（要被替换的原文）"))?;
            // new 允许空串（= 删除这段）；缺 new 才算参数错误
            let new = args
                .get("new")
                .and_then(Value::as_str)
                .ok_or_else(|| PluginError::invalid("replace_in_note 需要 new（替换成的文字，可为空串表示删除）"))?;
            let note = host::notes_get(id)?;
            if !note.body.contains(old) {
                return Err(PluginError::not_found(
                    "在笔记正文里找不到要替换的原文 old（请先 read_note 核对，或改用 update_note 整体替换）",
                ));
            }
            // 只替第一处匹配：选区文字通常唯一，避免误伤重复出现的同名内容
            let new_body = note.body.replacen(old, new, 1);
            let r = host::notes_upsert(id, None, Some(&new_body))?;
            Ok(json!({ "id": r.note.id, "title": r.note.title, "pending": r.pending, "hint": write_hint(r.pending) }))
        }
        other => Err(PluginError::invalid(format!(
            "未知工具: {other}（可用: search_notes/read_note/list_folders/replace_in_note/update_note/create_note）"
        ))),
    }
}
...
fn chat(args: Value) -> Result<Value, PluginError> {
    ...
    // 当前在笔记内容区选中的文字（宿主 §9.2，PluginSidebar 并入 args.selection.text）。
    // 有则注入系统提示词——让 AI 优先处理这段（改写走 replace_in_note）。
    let selection = args
        .get("selection")
    ...
    msgs.push(Message::system(system_prompt(note_id.as_deref(), selection.as_deref())));
    msgs.extend(history);
...
mod native_e2e {
    ...
    #[test]
    fn selection_is_injected_into_system_prompt() {
        setup_library();
        stub::set_ai_reply("好的，我来看看这段。");
        // args.selection.text（宿主 §9.2）→ 应进系统提示词，并提示可用 replace_in_note
        let args = json!({
            "messages": [{ "role": "user", "content": "帮我改写这段" }],
            "input": "帮我改写这段",
            "note_id": note_id(),
            "selection": { "text": "牛奶、面包" }
        });
        run_command("chat", args).unwrap();
        let msgs = stub::last_ai_messages().unwrap();
        let sys = msgs[0]["content"].as_str().unwrap();
        assert!(sys.contains("# 当前选中的文字") && sys.contains("牛奶、面包"), "选区应进系统提示词");
        assert!(sys.contains("replace_in_note"), "应提示用 replace_in_note 处理选区");
    }

    #[test]
    fn replace_in_note_replaces_first_match_and_persists() {
        setup_library();
        stub::set_write_pending(false); // 直写，断言落库
        stub::push_ai_reply(r#"{"tool":"replace_in_note","args":{"old":"牛奶","new":"酸奶"}}"#);
        stub::push_ai_reply("已把「牛奶」换成「酸奶」。");
        let out = run_command(
            "chat",
            chat_args(json!([{ "role": "user", "content": "把牛奶换成酸奶" }]), "把牛奶换成酸奶", Some(&note_id())),
        )
        .unwrap();
        assert_eq!(out["reply"], "已把「牛奶」换成「酸奶」。");
        // 只替第一处：牛奶→酸奶，其余不动
        assert_eq!(host::notes_get(&note_id()).unwrap().body, "酸奶、面包");
    }

    #[test]
    fn replace_in_note_missing_old_feeds_back_not_found() {
        setup_library();
        stub::set_write_pending(false);
        stub::push_ai_reply(r#"{"tool":"replace_in_note","args":{"old":"不存在的原文","new":"x"}}"#);
        stub::push_ai_reply("没找到要替换的文字。");
        let out = run_command(
            "chat",
            chat_args(json!([{ "role": "user", "content": "改一下" }]), "改一下", Some(&note_id())),
        )
        .unwrap();
        assert_eq!(out["reply"], "没找到要替换的文字。");
        // 正文不变
        assert_eq!(host::notes_get(&note_id()).unwrap().body, "牛奶、面包");
        let msgs = stub::last_ai_messages().unwrap();
        let arr = msgs.as_array().unwrap();
        let feedback = arr[arr.len() - 1]["content"].as_str().unwrap();
        assert!(feedback.contains("not_found"), "找不到 old 应以 not_found 回喂: {feedback}");
    }

    #[test]
    fn replace_in_note_respects_pending_proposal() {
        setup_library();
        stub::set_write_pending(true); // 提案模式：不落库
        stub::push_ai_reply(r#"{"tool":"replace_in_note","args":{"old":"面包","new":"吐司"}}"#);
        stub::push_ai_reply("已提交修改提案，请在弹窗中确认。");
        let out = run_command(
            "chat",
            chat_args(json!([{ "role": "user", "content": "把面包换成吐司" }]), "把面包换成吐司", Some(&note_id())),
        )
        .unwrap();
        assert_eq!(out["reply"], "已提交修改提案，请在弹窗中确认。");
        assert_eq!(host::notes_get(&note_id()).unwrap().body, "牛奶、面包", "提案不应落库");
        let msgs = stub::last_ai_messages().unwrap();
        let arr = msgs.as_array().unwrap();
        let feedback = arr[arr.len() - 1]["content"].as_str().unwrap();
        assert!(feedback.contains("\"pending\":true") || feedback.contains("\"pending\": true"));
